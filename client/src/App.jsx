...
/* ===== Utils (bỏ dấu * và ' trước khi so sánh/chấm/TTS) ===== */
...
/* ===== Hiển thị trọng âm từ dấu * (KHÔNG ảnh hưởng TTS) ===== */
function renderStress(s) {
  if (!s) return null;
  const out = [];
  let i = 0, key = 0;
  const isLetter = (ch) => /\p{L}/u.test(ch); // Unicode letter
  while (i < s.length) {
    if (s[i] === "*") {
      i++; // bỏ dấu *
      let j = i;
      ...
      out.push(<span className="stress" key={key++}>{chunk}</span>);
      i = j;
    } else {
      let j = i;
...
export default function App() {
  ...
  function getVoice(langPrefix = "en-US") {
    ...
    return list.find((v) => v.lang === langPrefix)
  ...
  function speakSample(accent = "us") {
    ...
    const u = new SpeechSynthesisUtterance(stripMarkers(t)); // <<< bỏ dấu * & ' trước khi đọc
  ...
  const submit = async (force = false) => {
    ...
    fd.append("text", stripMarkers(text)); // <<< gửi text đã bỏ dấu *
    ...
    if (!resp.ok) { console.error(payload); alert("Lỗi khi gọi /api/evaluate:\n" + JSON.stringify(payload, null, 2)); return; }
  ...
  return (
    <>
      <style>{`
        ...
        .chip:disabled{ opacity:.5; cursor:not-allowed; filter:grayscale(.3); }
        .chip:focus-visible{ outline:2px solid #2aa3ff; outline-offset:2px; }
      ...
      <div className="shell">
        ...
        <div className="text-picker-wrap">
          ...
          <div className="picked-text">
            {text ? renderStress(text) : "— Chưa có nội dung —"}
          </div>

          <div className="sample-voice">
            <button type="button" className="chip" title="Nghe mẫu giọng Mỹ (en-US)" onClick={(e) => { e.preventDefault(); speakSample("us"); }} disabled={!canHear}>🔊 US</button>
            <button type="button" className="chip" title="Nghe mẫu giọng Anh (en-GB)" onClick={(e) => { e.preventDefault(); speakSample("uk"); }} disabled={!canHear}>🔊 UK</button>
          </div>
