...
const API_BASE = import.meta.env.PROD
  ? window.location.origin                 // PROD: same-origin để tránh CORS
  : (import.meta.env.VITE_API_BASE || window.location.origin); // DEV: có thể trỏ http://localhost:3000/3001
...
export default function App() {
  // text không nhập trực tiếp; lấy từ file .txt
    ...
    (async () => {
      try {
        ...
        if (!ignore) {
          ...
          setSel(0);
          setText("");
          alert("Không thể đọc file nội dung (.txt) trong thư mục public.\nHãy tạo file /contents.txt (mỗi dòng 1 câu).");
        }
      }
    })();
    return () => { ignore = true; };
  ...
  const submit = async (force = false) => {
    ...
    fd.append("text", text.replace(/'/g, "")); // vẫn hỗ trợ đánh dấu trọng âm bằng '
  ...
  const selectByIndex = (idx) => {
    const i = Math.max(0, Math.min(idx, (lines.length || 1) - 1));
    setSel(i);
    setText(lines[i] || "");
    setResult(null);
    setAsrTranscript("");
  ...
  return (
    <>
      <style>{`
        :root{
          --bg:#0b1220;
          --text:#eaf2ff; --muted:#9fb0c3; --border:rgba(255,255,255,.10);
        ...
        *, *::before, *::after { box-sizing: border-box; }
        body{
          ...
          color: var(--text);
          background-image:
            radial-gradient(ellipse at 20% 0%, rgba(0,0,0,.35), rgba(0,0,0,.65)),
            url("/hinh-nen-ai-61.jpg");
        ...
        .picker-row .select{
          min-width:260px; padding:8px 10px; border-radius:10px;
          background:#0a1220; color:var(--text); border:1px solid var(--border);
        }
        .picker-row .btn{ padding:8px 12px; }
        .picked-text{
          padding:14px; border-radius:12px; border:1px solid var(--border);
          background:#0a1220; min-height:72px; line-height:1.55;
          white-space:pre-wrap;
        }
        .sample-voice{position:absolute; left:16px; bottom:16px; display:flex; gap:8px;}
        .sample-voice .btn{font-size:13px; padding:6px 10px;}

        .robot3d{
          position:absolute; right:6px; bottom:8px; width:170px; height:170px;
          pointer-events:none; user-select:none;
          filter: drop-shadow(0 8px 18px rgba(0,0,0,.35));
        }
        .robot3d svg{ width:100%; height:100%; display:block; }
        @keyframes rbTalk { 0%,100%{ transform: scaleY(.25);} 50%{ transform: scaleY(1);} }
        @keyframes rbBob  { 0%,100%{ transform: translateY(0);} 50%{ transform: translateY(-2px);} }
        @keyframes rbGlow { 0%,100%{ opacity:.6;} 50%{ opacity:1;} }
        .robot3d .rb-head{ transform-origin: 140px 88px; animation: rbBob 2.2s ease-in-out infinite; }
        .robot3d .rb-mouth{ transform-origin: 140px 112px; animation: rbTalk .28s ease-in-out infinite; animation-play-state: paused; }
        .robot3d.talking .rb-mouth{ animation-play-state: running; }
        .robot3d.talking .rb-eyes{ animation: rbGlow .8s ease-in-out infinite; }

        @media (max-width: 680px){
          .text-picker-wrap{ padding-bottom: 64px; }
          .robot3d{
            position:relative; right:auto; bottom:auto;
            width:130px; height:130px; margin:10px auto 4px;
            filter: drop-shadow(0 6px 14px rgba(0,0,0,.35));
          }
          .sample-voice{ left:12px; bottom:12px; }
        }
        @media (max-width: 360px){
          .robot3d{ width:110px; height:110px; }
        }
      ...
      <div className="shell">
        ...
        <p className="subtitle">
          Nội dung lấy từ file <code>{CONTENT_FILE}</code> (mỗi dòng 1 câu). Có thể đánh dấu <b>trọng âm</b> bằng dấu nháy đơn (<code>'</code>).
        </p>
        ...
        <div className="text-picker-wrap">
          <div className="picker-row">
            ...
            <button className="btn" onClick={() => selectByIndex(sel - 1)} disabled={!lines.length || sel <= 0}>◀ Trước</button>
            <button className="btn" onClick={() => selectByIndex(sel + 1)} disabled={!lines.length || sel >= lines.length - 1}>Sau ▶</button>
            <button className="btn" onClick={() => { if (lines.length) selectByIndex(Math.floor(Math.random() * lines.length)); }} disabled={!lines.length}>
              🎲 Ngẫu nhiên
            </button>
          </div>
          ...
          <div className="sample-voice">
            <button type="button" className="btn" title="Nghe mẫu giọng Mỹ (en-US)" onClick={(e) => { e.preventDefault(); speakSample("us"); }}>
              🔊 US
            </button>
            <button type="button" className="btn" title="Nghe mẫu giọng Anh (en-GB)" onClick={(e) => { e.preventDefault(); speakSample("uk"); }}>
              🔊 UK
            </button>
          </div>
        ...
        <tbody>
          {words.map((w, i) => (
            <tr key={i}>
              ...
              {on("vocab") && <td className="muted">0/1 dựa trên dấu <code>'</code> đã đánh trong dòng nội dung</td>}
              {on("phones") && (
                <td>
