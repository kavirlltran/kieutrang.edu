...
export default function App() {
  ...
  const submit = async (force = false) => {
    ...
    fd.append("text", stripMarkers(text));         // bỏ * và ' trước khi gửi
  ...
  return (
    <>
      <style>{`
        ...
        @media (max-width: 680px){ .text-picker-wrap{ padding-bottom:64px; } .robot3d{ position:relative; right:auto; bottom:auto; width:130px; height:130px; margin:10px auto 4px; } .sample-voice{ left:12px; bottom:12px; } }
      `}</style>
      ...
      <div className="shell">
        ...
        {/* chips giữ nguyên code của bạn */}

        <div className="text-picker-wrap">
          <div className="picker-row">
            <label htmlFor="contentSelect" className="muted">Chọn nội dung:</label>
            <select id="contentSelect" className="select" value={sel} onChange={(e)=>selectByIndex(Number(e.target.value))} disabled={!lines.length}>
              {lines.length
                ? lines.map((s,i)=><option key={i} value={i}>{`${i+1}. ${s.length>80?s.slice(0,80)+'…':s}`}</option>)
            ...
            <button className="btn" onClick={()=>{ if (lines.length) selectByIndex(Math.floor(Math.random()*lines.length)); }} disabled={!lines.length}>🎲 Ngẫu nhiên</button>
          </div>

          <div className="picked-text">{text || "— Chưa có nội dung —"}</div>

          <div className="sample-voice">
            <button type="button" className="btn" title="Nghe mẫu giọng Mỹ (en-US)" onClick={(e)=>{e.preventDefault();speakSample("us");}}>🔊 US</button>
            <button type="button" className="btn" title="Nghe mẫu giọng Anh (en-GB)" onClick={(e)=>{e.preventDefault();speakSample("uk");}}>🔊 UK</button>
          </div>

          <Robot3D talking={robotTalking} />
        </div>

        {/* actions + audio + result giữ nguyên code hiện tại */}
        <div className="actions">
          <button className="btn" onClick={recording ? stopRecording : startRecording}>{recording ? "⏹ Stop" : "🎤 Start Recording"}</button>
          <label className="btn" style={{ cursor: "pointer" }}>
            <input type="file" accept="audio/*" hidden onChange={onPickFile} /> Chọn file WAV/WebM
          </label>
        ...
        {asrEnabled && (asrFinal || asrTranscript) && (<p className="muted" style={{ marginTop: 6 }}><b>ASR:</b> {(asrFinal + " " + asrTranscript).trim()}</p>)}
        {gateWarn && (
